  ...
  return (
    <div className="min-h-screen bg-bg-main p-6 safe-area-bottom pb-32">
      <header className="flex items-center justify-between mb-8">
        ...
        <h1 className="text-xl font-black italic uppercase tracking-tighter text-text-primary">Care Circle</h1>
        <button 
      ...
      <div className="max-w-md mx-auto space-y-8">
        <section className="text-center space-y-3">
          ...
          <h2 className="text-2xl font-black tracking-tight uppercase italic text-text-primary">Family Monitoring</h2>
          <p className="text-text-secondary text-sm font-medium">
            Your trusted circle can monitor your adherence and help in emergencies.
          </p>
        </section>

        <div className="flex gap-2 p-1 bg-surface-main rounded-2xl border border-border-main">
          {["Members", "Permissions", "Logs"].map((tab) => (
            <button 
              ...
              className={cn(
                ...
                activeTab === tab.toLowerCase() ? "bg-primary text-black shadow-lg" : "text-text-secondary hover:text-text-primary"
              )}
            >
              {tab}
            </button>
             ...
             >
                <div className="flex items-center gap-4">
                   <div className="relative">
                      <div className="w-12 h-12 rounded-2xl bg-bg-main border border-border-main flex items-center justify-center text-text-primary font-black">
                         {member.name[0]}
                      </div>
                      <div className={cn(
                        "absolute -bottom-1 -right-1 w-3 h-3 rounded-full border-2 border-surface-main",
                        member.status === "online" ? "bg-safe" : "bg-text-secondary opacity-40"
                      )} />
                   </div>
                   <div>
                      <h3 className="font-bold text-text-primary tracking-tight">{member.name}</h3>
                      <p className="text-[10px] font-black uppercase tracking-widest text-text-secondary opacity-60">{member.relationship} • {member.role}</p>
                   </div>
                </div>
                ...
                <div className="flex items-center gap-2">
                   <button 
                     ...
                     className="p-2.5 rounded-xl bg-surface-main border border-border-main text-text-secondary hover:text-primary transition-colors"
                   >
                      <MessageSquare size={18} />
                   </button>
                   <button className="p-2.5 rounded-xl bg-surface-main border border-border-main text-text-secondary hover:text-primary transition-colors">
                      <Phone size={18} />
                   </button>
                   <button className="p-2.5 rounded-xl bg-surface-main border border-border-main text-text-secondary hover:text-primary transition-colors">
                      <Video size={18} />
                   </button>
                   <button className="p-2.5 rounded-xl bg-surface-main border border-border-main text-text-secondary">
        ...
        <div className="p-6 bg-ai/5 border border-ai/20 rounded-[32px] space-y-4">
           <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-2xl bg-ai flex items-center justify-center text-black">
                 <Sparkles size={20} />
              </div>
              <div>
              ...
              >
                 <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                       ...
                       <h3 className="text-xl font-black uppercase italic text-text-primary">Add Member</h3>
                    </div>
                    <button onClick={() => setShowInviteModal(false)} className="p-2 text-text-secondary">
                       <X size={20} />
                    </button>
                 </div>
                 ...
                 <div className="space-y-6">
                    <div className="space-y-2">
                       <label className="text-[10px] font-black uppercase tracking-widest text-text-secondary opacity-60 ml-1">Email Address</label>
                       <input 
                         value={inviteEmail}
                         onChange={e => setInviteEmail(e.target.value)}
                         placeholder="family@example.com" 
                         className="w-full h-14 bg-surface-main border border-border-main rounded-2xl px-6 text-sm font-bold text-text-primary outline-none focus:border-primary placeholder:text-text-secondary/40" 
                       />
                    </div>
                    
                    <div className="space-y-2">
                       <label className="text-[10px] font-black uppercase tracking-widest text-text-secondary opacity-60 ml-1">Role</label>
                          ...
                          {["Caregiver", "Family"].map(r => (
                            <button 
                              ...
                              className={cn(
                                ...
                                inviteRole === r.toLowerCase() ? "bg-primary text-black" : "bg-surface-main border border-border-main text-text-secondary"
                              )}
                            >
                               {r}
                            </button>
