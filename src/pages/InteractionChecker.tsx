  ...
  return (
    <div className="min-h-screen bg-bg-main p-6 safe-area-bottom">
      <header className="flex items-center justify-between mb-8">
        ...
        <h1 className="text-xl font-black italic uppercase tracking-tighter text-text-primary">Interaction AI</h1>
        <div className="w-10" />
      </header>
      ...
      <div className="max-w-md mx-auto space-y-8">
        <section className="text-center space-y-3">
          ...
          <h2 className="text-2xl font-black tracking-tight uppercase italic text-text-primary">Safe-Check Engine</h2>
        ...
        <div className="relative group">
          ...
          <input 
            ...
            className="w-full h-16 bg-surface-main border border-border-main rounded-2xl pl-14 pr-6 font-bold text-text-primary placeholder:text-text-secondary/50 focus:border-primary focus:ring-1 focus:ring-primary transition-all outline-none"
            onKeyDown={(e) => e.key === 'Enter' && handleCheck()}
          />
          <button 
            onClick={handleCheck}
            disabled={!query || loading}
            className="absolute right-3 top-3 bottom-3 px-6 bg-primary text-black rounded-xl font-black uppercase text-[10px] tracking-widest disabled:opacity-50"
          >
        ...
        <div className="space-y-4">
           ...
           <div className="flex flex-wrap gap-2">
              {medications.map(m => (
                <span key={m.id} className="px-3 py-1.5 bg-surface-main border border-border-main rounded-lg text-[10px] font-bold text-text-primary">{m.name}</span>
              ))}
              {medications.length === 0 && <p className="text-[10px] italic text-text-secondary/60">No medications active.</p>}
           </div>
        </div>
          ...
          {result && (
            ...
            >
               <div className="flex items-center gap-4">
                  ...
                  <div>
                     <h4 className="text-lg font-black uppercase italic leading-none text-text-primary">{result.safe ? "Safe to Proceed" : "Potential Risk"}</h4>
                 ...
                 <div className="space-y-2">
                    ...
                    <div className="space-y-2">
                       {result.warnings.map((w: string, i: number) => (
                         <div key={i} className="flex gap-3 items-start text-xs text-text-secondary">
               ...
               <div className="pt-4 flex gap-3">
                  <button className="flex-1 py-3 bg-surface-main border border-border-main rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-black/5 dark:hover:bg-white/5 transition-all text-text-primary">Save Report</button>
                  <button onClick={() => navigate('/chat')} className="flex-1 py-3 bg-primary text-black rounded-xl text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-2">
